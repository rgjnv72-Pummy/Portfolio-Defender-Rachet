...
def send_telegram_text(results):
    # Try multiple common secret names to ensure a match
    token = os.getenv('TELEGRAM_TOKEN') or os.getenv('TOKEN') or os.getenv('BOT_TOKEN')
    chat_id = os.getenv('CHAT_ID') or os.getenv('TELEGRAM_CHAT_ID')
    
    print(f"📡 Secret Scan -> Token Found: {bool(token)} | Chat ID Found: {bool(chat_id)}")

    if not token or not chat_id:
        print("❌ ERROR: No secrets found in Environment. Please check your YML 'env:' block.")
        return
    ...
    try:
        r = requests.post(url, json={'chat_id': str(chat_id).strip(), 'text': msg, 'parse_mode': 'Markdown'}, timeout=20)
        print(f"📊 Telegram Response: {r.status_code}")
        if r.status_code != 200: print(f"❌ Error Detail: {r.text}")
